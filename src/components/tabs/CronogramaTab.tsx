...
export default function CronogramaTab({
  ...
  const filteredArchitects = ARCHITECTS.filter(arch => 
    arch.name.toLowerCase().includes(archQuery.toLowerCase())
  );
  ...
  const handleUnlockPreviousWeeks = () => {
    ...
    setTimeout(() => {
      ...
      if (target && desktopContainer) {
        if (desktopContainer.contains(target)) {
      ...
      <div className="sticky -top-4 md:-top-8 -mx-4 md:-mx-8 px-4 md:px-8 pt-1 md:pt-2 pb-2 z-20 bg-[#F8F9FB]">
        <div className="bg-white border border-slate-200/60 p-3 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-3 shadow-sm">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full md:w-auto relative">
            <span className="text-xs font-bold uppercase text-slate-500 tracking-wider">Arquiteto Responsável:</span>
            <div className="relative inline-block text-left w-full sm:w-64">
              <button
                ...
                className="flex items-center justify-between w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-700 cursor-pointer hover:bg-slate-50 transition-all shadow-sm focus:outline-none"
              >
                  ...
                  <div className="py-1 max-h-48 overflow-y-auto">
                    {filteredArchitects.map(arch => (
                      <button
                        ...
                        className={`flex items-center justify-between w-full px-4 py-2.5 text-xs font-bold hover:bg-slate-50 transition-colors cursor-pointer text-left ${
          ...
          <div className="flex items-center gap-2.5 w-full md:w-auto justify-end border-t md:border-t-0 pt-2.5 md:pt-0 border-slate-100">
            <span className="text-xs font-bold uppercase text-slate-500 tracking-wider">Ano:</span>
            <select
              value={selectedYear}
              onChange={(e) => {
              ...
              className="bg-white border border-slate-200 rounded-xl px-4 py-2 text-xs font-bold text-slate-700 cursor-pointer focus:outline-none hover:bg-slate-50 transition-colors shadow-sm"
            >
        ...
        {!showPreviousWeeks && (
          <div className="flex justify-center py-2 animate-fade-in">
            <button
              ...
              className="px-6 py-2.5 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 rounded-xl text-xs font-bold cursor-pointer transition-all shadow-sm flex items-center gap-2"
            >
            ...
            >
              ...
              <div className={`p-3 px-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 ${
                ...
                <div className="flex items-center gap-2">
                  {isCurrentWeek && (
                    <span className="text-[8px] font-extrabold tracking-widest px-2 py-0.5 rounded-full bg-white text-slate-950">
                      SEMANA ATUAL
                    </span>
                  )}
                  <h3 className="font-bold text-xs tracking-wide uppercase">{formatWeekLabel(week.startDate, week.endDate)}</h3>
                </div>
                        ...
                        >
                          ...
                          <td className="p-1 px-3 text-center">
                            <div className="flex items-center justify-center gap-1">
                              <button
                                onClick={() => isTaskEmpty ? triggerToast('Linha vazia!', 'warning') : copyTaskToBuffer(task as Task)}
