    ...
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200/60 rounded-2xl p-5 flex flex-col justify-between hover:shadow-sm transition-all">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider font-mono">🖥️ Total de Ativos</span>
          <div className="my-2 text-2xl font-bold text-slate-800">{totalAssets}</div>
          <span className="text-[9px] font-mono text-slate-400">Máquinas e utilizadores registrados</span>
        </div>
        
        <div className="bg-white border border-slate-200/60 rounded-2xl p-5 flex flex-col justify-between hover:shadow-sm transition-all">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider font-mono">💿 Mix de Softwares</span>
          <div className="my-2 text-2xl font-bold text-slate-800">{distinctSoftwaresCount}</div>
          <span className="text-[9px] font-mono text-slate-400 truncate" title={distText}>{distText || 'Nenhum software'}</span>
        </div>

        <div className="bg-white border border-slate-200/60 rounded-2xl p-5 flex flex-col justify-between hover:shadow-sm transition-all">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider font-mono">✅ Licenciados</span>
          <div className="my-2 text-2xl font-bold text-slate-800">{licensedCount}</div>
          <span className="text-[9px] font-mono text-slate-400">Instalações ativas regulares</span>
        </div>

        <div className="bg-white border border-slate-200/60 rounded-2xl p-5 flex flex-col justify-between hover:shadow-sm transition-all">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider font-mono">⚠️ Alertas / Breves</span>
          <div className="my-2 text-2xl font-bold text-slate-800">{criticalCount}</div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col justify-between space-y-4">
        <div className="flex flex-col gap-4 border-b border-slate-100 pb-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <button
              ...
              className="px-4 py-2.5 bg-slate-900 text-white rounded-xl text-xs font-bold hover:bg-slate-800 transition-all flex items-center gap-1.5 cursor-pointer shadow"
            >
              ...
              {licenseViewMode === 'complete' && (
                <>
                  <tr className="bg-slate-50/50 text-slate-400 uppercase font-mono text-[9px] border-b border-slate-200/60">
                    ...
                    <th colSpan={6} className="p-1.5 border-b border-r border-slate-200/60 text-center font-bold tracking-wider bg-slate-100 text-slate-600">Licenciamento de Softwares</th>
                      ...
                      <td className="p-1 border-r border-slate-200/60">
                        <div className="flex flex-col gap-1">
                          <select
                            ...
                            className="bg-transparent border-none p-1 rounded cursor-pointer text-[11px] font-bold focus:ring-0 focus:bg-slate-50 focus:outline-none text-slate-700"
                          >
                          ...
                          {(!['Revit', 'AutoCAD', 'SketchUp Pro', 'Archicad', 'Lumion', 'TQS', 'V-Ray'].includes(lic.software || '') || !lic.software) && (
                            <input
                              ...
                              className="bg-slate-50 border-none p-1 rounded font-bold text-[10px] w-full focus:outline-none"
                    ...
                    {licenseViewMode === 'expiring' && (
                      <td className="p-1.5 border-r border-slate-200/60 text-center">
                        {lic.revitStatus === 'Sem Licença' ? (
                          <span className="px-2 py-0.5 rounded text-[8px] font-bold bg-rose-50 text-rose-700 border border-rose-200 uppercase">
                            🚨 Sem Licença
                          </span>
                        ) : isNearExpiry ? (
                          <span className="px-2 py-0.5 rounded text-[8px] font-bold bg-amber-50 text-amber-700 border border-amber-200 uppercase animate-pulse">
                            ⏳ Expira Breve (2026)
                          </span>
                        ) : (
                          <span className="px-2 py-0.5 rounded text-[8px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 uppercase">
                            ✅ Regular
                          </span>
                        )}
                      </td>
        ...
        <div className="bg-slate-900 text-white p-3.5 px-5 font-mono text-[9px] flex flex-col sm:flex-row items-center justify-between gap-2 rounded-xl">
          <div>COLABORADORES ATIVOS: <span className="font-bold">{licenses.length} REGISTROS</span></div>
